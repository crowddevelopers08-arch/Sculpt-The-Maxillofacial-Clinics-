...
interface LeadData {
  ...
  source?: string
  formName?: string
  service?: string // New field for hair transplant form
}
...
function generateFormDataString(leadData: LeadData): string {
  ...
  if (leadData.service) details.push(`Service: ${leadData.service}`);
  ...
  try {
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        procedure: leadData.procedure || leadData.treatment || leadData.service || '',
        message: leadData.message || '',
        city: leadData.city || '',
        age: leadData.age || '',
        preferredDate: leadData.preferredDate || leadData.date || '',
  ...
  try {
    ...
    const telecrmPayload = {
      fields: {
        ...
        message: leadData.message || "",
        select_the_procedure: leadData.procedure || leadData.treatment || leadData.service || "",
        Country: "",
        LeadID: "",
      ...
      actions: [
        ...
        {
          "type": "SYSTEM_NOTE",
          "text": `Procedure/Treatment/Service: ${leadData.procedure || leadData.treatment || leadData.service || 'Not specified'}`
        },
        {
          "type": "SYSTEM_NOTE",
...
/**
 * Handle POST request for all forms
 */
      ...
      try {
        ...
        const errorLead = await prisma.lead.create({
          data: {
            ...
            phone: data.phone,
            email: data.email || '',
            procedure: data.procedure || data.treatment || data.service || '',
